Shader "IridescencePack/IridescentScan"
{
    Properties
    {
        [Header(TEXTURES)]
        ...
        [Space(10)]
        _SwapThreshold ("Swap Threshold", Float) = 0.5
        [HDR] _DissolveColor ("Dissolve Color", Color) = (1,1,1,1)
        _HologramColor ("Hologram Color", Color) = (1,1,1,1)
        _HoloFresnelIntensity("Fresnel Intensity", Range(0, 10)) = 0 
        _HoloFresnelRamp("Fresnel Ramp", Range(0, 10)) = 0 
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 _GlossColor;
            float _Transparency;
            float _SwapThreshold;
            float3 _DissolveColor;
            float3 _HologramColor;
            float _HoloFresnelIntensity;
            float _HoloFresnelRamp;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // holo fresnel
                float holoFresnelAmount = 1 - max(0.0, dot(normal, i.viewDir));
                holoFresnelAmount = pow(holoFresnelAmount, _HoloFresnelRamp) * _HoloFresnelIntensity;


                // dissolve
                float modifiedXPos = i.objectPos.x + _SwapThreshold;
                float modifiedYPos = step(0, sin(modifiedXPos * 10)) + step(0, sin(i.objectPos.x * 7 - _Time.y * 5));
                float yPos = i.objectPos.y + modifiedYPos / 5;
                
                if (yPos > _SwapThreshold)
                {
                    finalColor = _DissolveColor;
                }
                if (yPos > _SwapThreshold + 0.02)
                {
                    float3 holo = _HologramColor * holoFresnelAmount;
                    finalColor = holo * step(0, sin(i.vertex.y * 10 + _Time.y * 2));
                }
