Shader "USB/USB_function_SinCos_Modified"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
        _Speed ("Speed", Range(0, 3)) = 1
        _TwistAmount ("TwistAmount", Range(0, 1)) = 0.3
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 rotation(float3 vertex, float2 uv)
            {
                _Speed += (uv.y * _TwistAmount);
            ...
            {
                v2f o;
                float3 rotVertex = rotation(v.vertex, v.uv);
