Shader "IridescencePack/IridescentMasked"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _MainTex, _NormalMap, _Mask;
            float4 _MainTex_ST;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float3 ir = grad;
                ir = ir + directSpecular;
                ir = (ir + directSpecular) * max(float3(_Shading, _Shading, _Shading), (diffuseLight + fresnelAmount / 4) * (ambient * 2) + (fresnelAmount * diffuseLight));

                float3 main = tex2D(_MainTex, i.uv);
                float3 mask = tex2D(_Mask, i.uv);

                float3 finalIr = lerp(float3(0.0, 0.0, 0.0), ir, mask.r);

                float3 final;
                if (mask.r > 0.1)
                {
                    final = ir * diffuseLight + directSpecular;
                }
                else
                {
                    final = main * diffuseLight + directSpecular;
                }

                

                // float3 m = main * diffuseLight + directSpecular;

                float alpha = lerp(1.0, fresnelAmount, _Transparency);
                
                return fixed4(final, alpha);
            }
            ENDCG
        }
    }
