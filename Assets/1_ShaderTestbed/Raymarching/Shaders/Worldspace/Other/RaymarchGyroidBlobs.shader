Shader "Raymarch/GyroidBlobs"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
            	float rescaleFactor = 35.0;
	            p *= rescaleFactor;

            	float thickness = 0.001;

            	thickness = thickness + sin(_Time.y) * 0.005 + 0.03; // pulse
			...
			float getDist(float3 p)
            {
            	...
            	float ball1 = abs(sphere1(p)) - 0.01;
            	float ball2 = abs(sphere2(p)) - 0.01;
            	float ball3 = abs(sphere3(p)) - 0.01;
				float g = ballGyroid(p);

            	float k = 0.05;

            	// ball = max(ball, g); // boolean intersection
            	ball1 = smin(ball1, g, -k); // negative k value turns smin into smax
            	ball2 = smin(ball2, g, -k);
            	ball3 = smin(ball3, g, -k);
            	float combined1 = smin(ball1, ball2, k);
            	float combined2 = smin(combined1, ball3, k);
