Shader "Raymarch/TwistedTorus02"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
    	_SmoothAmount ("Smooth Amount", Range(0, 1.0)) = 0.1
    	...
    	_TorusSize("Torus Size", Float) = 1.0
    	_DistanceCorrection("Distance Correction", Float) = 0.05
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _Twist, _TorusSize;
            float _DistanceCorrection;
            float _Speed;
            ...
            {
            	float d = length(float2(length(p.xz) - .4 * _TorusSize, p.y)) - .1 * _TorusSize; // only works with flat torus
            	return d;
            }
            ...
            {
	            float k = _Twist;
            	float c = cos(k * p.y + _Time.y * _Speed + sin(_Time.y) * 0.2);
            	float s = sin(k * p.y + _Time.y * _Speed + cos(_Time.y) * 0.3);
