Shader "Raymarch/LitSpheresDifferentMaterials"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float sphere(float3 p, float3 worldPos, float r)
            {
            	p -= worldPos;
	            float d = length(p) - r;
            	return d;
            ...
            {
            	float s1 = sphere(p, 0.2, 0.2);
            	float s2 = sphere(p, -0.2, 0.2);
            	float d = min(s1, s2);
				return d;
			}
			...
			int getID(float3 p)
            {
            	...
            	float s1 = sphere(p, 0.2, 0.2);
            	float s2 = sphere(p, -0.2, 0.2);
            	// float d = smin(s1, s2, 0.1); // doesnt work with smooth minimum
            	float d = min(s1, s2);
